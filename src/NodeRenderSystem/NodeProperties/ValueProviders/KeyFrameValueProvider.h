...
public:
    ...
    using KeyFramGroupType = KeyFrameGroup<T>;

    KeyFrameValueProvider(const KeyFramGroupType& keyframe_group)
        : m_keyframes(keyframe_group.m_keyframes)
    {
    }
    ...
    bool has_update(eao::FrameType time) const override
    {
        ...
        if (is_valid_index(m_leading_index) and is_valid_index(m_trailing_index) and m_keyframes[m_leading_index].m_is_hold)
        {
            if ((m_keyframes[m_leading_index].m_time < time) and (time < m_keyframes[m_trailing_index].m_time))
                return true;
        }
    ...
    const valueType& value(eao::FrameType time) override
    {
        ...
        {
            // interpolate
            double progress = m_keyframes[m_leading_index].interpolated_progress(m_keyframes[m_trailing_index], time);
            m_value = m_keyframes[m_leading_index].interpolate_to(m_keyframes[m_trailing_index], progress);
        }
...
private:
    ...
    {
        return (index >= 0) and index < m_keyframes.size();
    }
private:
    T m_value;

    const QList<KeyFrameType>& m_keyframes;
